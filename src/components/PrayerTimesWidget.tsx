...
export function PrayerTimesWidget({
  ...
  const [deviceCoords, setDeviceCoords] = useState<GeoCoordinate | null>(
    storedCoords,
  );
  ...
  const reloadTimes = useCallback(async () => {
    ...
    cache.aladhanMinutes,
    cache.londonFeedMinutes,
    selectedLocation,
    selectedMadhab,
  ]);
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem(
      LAST_DEVICE_COORDS_KEY,
      JSON.stringify(deviceCoords),
    );
  }, [deviceCoords]);
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem(
      MADHAB_OVERRIDES_KEY,
      JSON.stringify(madhabOverrides),
    );
  }, [madhabOverrides]);
  ...
  useEffect(() => {
    ...
    setSelectedWeekDate((current) => {
      ...
      const defaultDate =
        weekSchedule.find((day) => day.date === timeline?.nextDate)?.date ??
  ...
  const selectedWeekDay = useMemo(() => {
    ...
    return (
      weekSchedule.find((day) => day.date === selectedWeekDate) ??
      weekSchedule[0]
    );
  ...
  const handleUseLocation = (): void => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => {
        setErrorMessage(
          "Could not access your location. Using timezone-based fallback instead.",
        );
        setIsLocating(false);
      },
      {
        ...
        <>
          ...
          {timeline ? (
            <div className="prayer-callout">
              <p>
                Now: <strong>{timeline.current.name}</strong> (
                {formatPrayerClock(timeline.current.time24, use24Hour)})
              </p>
              <p>
                Next: <strong>{timeline.next.name}</strong> (
                {formatPrayerClock(timeline.next.time24, use24Hour)}) in{" "}
          ...
          {weekSchedule.length > 1 ? (
            <div className="prayer-week-panel">
              ...
              {selectedWeekDay ? (
                <p className="source-note">
                  Viewing: {formatPrayerDateLabel(selectedWeekDay.date)} (
