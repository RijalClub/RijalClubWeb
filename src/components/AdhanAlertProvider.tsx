...
import { loadPrayerTimesForLocation, pickRecommendedLocation, type PrayerTimesSnapshot } from '@/lib/prayer'
import type { AdhanMadhab, LinksConfig, PrayerCacheConfig, PrayerConfig } from '@/types/content'
...
function findDuePrayer(snapshot: PrayerTimesSnapshot, windowSeconds: number, now = new Date()): DuePrayer | null {
...
function findNextPrayerTrigger(snapshot: PrayerTimesSnapshot, now = new Date()): NextPrayerTrigger | null {
...
export function AdhanAlertProvider({ links, prayerConfig, prayerCache, children }: AdhanAlertProviderProps) {
  ...
  const reloadPrayerSnapshot = useCallback(async (): Promise<PrayerTimesSnapshot> => {
    const timezone = Intl.DateTimeFormat().resolvedOptions().timeZone || 'UTC'
    const storedCoords = loadStoredDeviceCoords()
    const useDeviceCoords = typeof window !== 'undefined' && window.localStorage.getItem(USE_DEVICE_COORDS_KEY) === 'true'
    const coordinateOverride = useDeviceCoords && storedCoords ? storedCoords : undefined

    const fallbackLocation = pickRecommendedLocation(prayerConfig, timezone, coordinateOverride)

    const selectedLocationId = typeof window === 'undefined' ? null : window.localStorage.getItem(SELECTED_LOCATION_KEY)
    const selectedLocation = prayerConfig.locations.find((location) => location.id === selectedLocationId) ?? fallbackLocation
    ...
    const selectedMadhab: AdhanMadhab | undefined =
      selectedLocation.provider === 'iccuk_html'
        ? undefined
        : (madhabOverrides[selectedLocation.id] ?? selectedLocation.adhanMadhab ?? 'shafi')
  ...
  }, [prayerCache.aladhanMinutes, prayerCache.londonFeedMinutes, prayerConfig])
  ...
  useEffect(() => {
    ...
    const runReload = async (): Promise<void> => {
      ...
      try {
        ...
        if (!disposed) {
          setStatusMessage('Unable to refresh prayer schedule. Retrying shortly.')
        }
      } finally {
        running = false
      }
  ...
  useEffect(() => {
    ...
    const syncPreferences = (): void => {
      ...
      setIsAdhanAlertEnabled(window.localStorage.getItem(ADHAN_ALERT_ENABLED_KEY) !== 'false')
  ...
  return (
    <AdhanAlertContext.Provider value={contextValue}>
      ...
      {adhanAlert?.enabled ? <audio ref={audioRef} src={adhanAlert.audioUrl} preload="none" /> : null}

      {showMiniPlayer ? (
        <section className="adhan-mini-player panel" aria-label="Adhan mini player">
          ...
          <div className="adhan-mini-actions">
            ...
            <button type="button" className={`icon-btn ${isMuted ? 'active' : ''}`} onClick={toggleMuted}>
              {isMuted ? <VolumeX size={14} /> : <Volume2 size={14} />}
              {isMuted ? 'Muted' : 'Mute'}
            </button>
          </div>
          {statusMessage ? <p className="source-note adhan-mini-status">{statusMessage}</p> : null}
        </section>
      ) : null}
