...
import type { MediaAsset, ProfileConfig, StoreConfig, StoreProduct } from '@/types/content'
    ...
    <>
      <main className="page-grid">
        <section className="panel reveal store-hero">
          ...
          <p className={store.isOpen ? 'store-status open' : 'store-status closed'}>
            {store.isOpen ? 'Store is open' : store.closedMessage}
          </p>
          {!store.isOpen ? (
            <a href={profile.secondaryCta.url} target="_blank" rel="noopener noreferrer" className="btn btn-ghost">
              Stay Updated
            </a>
          ) : null}
        </section>
