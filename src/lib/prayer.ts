...
function resolveAdhanHighLatitudeRule(
  location: PrayerLocation,
...
function getNowLabelInTimezone(
  timezone: string,
  now: Date,
  use24Hour: boolean,
): string {
  ...
  for (const route of config.timezoneRoutes) {
    if (timezone.startsWith(route.startsWith)) {
      const match = config.locations.find(
        (location) => location.id === route.locationId,
      );
      if (match) {
        return match;
      }
  ...
  switch (prayer) {
    ...
    case "Asr":
      return madhab === "hanafi"
        ? (day.asr_hanafi ?? day.asr ?? "")
        : (day.asr ?? day.asr_hanafi ?? "");
    case "Maghrib":
...
async function loadLondonUnified7dTimes(
  ...
  const payload = await fetchJsonWithCache<LondonUnifiedPayload>(
    location.officialSourceUrl,
    {
      ttlMs,
...
async function loadAdhanTimes(
  ...
  for (const dateKey of targetDates) {
    const prayers = buildAdhanDayPrayers(
      location,
      dateKey,
      coordinateOverride,
      adhanMadhabOverride,
  ...
  const selectedDay =
    weekSchedule.find((day) => day.date === todayKey) ?? weekSchedule[0];
...
export async function loadPrayerTimesForLocation(
  ...
  return loadAdhanTimes(
    location,
    options?.coordinateOverride,
    options?.adhanMadhabOverride,
  );
}
...
export function buildPrayerTimeline(
  ...
  if (weekSchedule && weekSchedule.length > 0) {
    ...
    if (todayIndex >= 0) {
      ...
      if (todaySlots.length > 0) {
        ...
        const todayCurrent = [...todaySlots]
          .reverse()
          .find((slot) => slot.minutes <= nowMinutes);
        ...
        const currentSlot = (() => {
          ...
          return (
            previousSlots[previousSlots.length - 1] ??
        ...
        if (todayNext) {
          return {
            ...
            nextDate: today.date,
            nowLabel: getNowLabelInTimezone(
        ...
        return {
          ...
          minutesUntilNext:
            dayGap * 24 * 60 - nowMinutes + nextDayFirst.minutes,
        };
      }
    }
  }
  ...
  const next =
    withMinutes.find((slot) => slot.minutes > nowMinutes) ?? withMinutes[0];
  const current =
    [...withMinutes].reverse().find((slot) => slot.minutes <= nowMinutes) ??
    withMinutes[withMinutes.length - 1];

  const minutesUntilNext =
    next.minutes > nowMinutes
      ? next.minutes - nowMinutes
      : 24 * 60 - nowMinutes + next.minutes;
